...
//    goActionDocFiles = new QMap<QAction*,QString>;
	...
	prjstat = new ProjectStatistics(this);
    autotxt = new AutoText(this);
	...
	connect(prjstat, SIGNAL(copyAllImages()), helpDock, SLOT(copyAllImages()));
    connect(prjstat, SIGNAL(getAllStat()), helpDock, SLOT(getAllStat()));
	connect(helpDock, SIGNAL(setStatCountImages(int,int,int)), prjstat, SLOT(setStatCountImages(int,int,int)));
	connect(helpDock, SIGNAL(setStatImageLists(QStringList,QStringList,QStringList)), prjstat, SLOT(setStatImageLists(QStringList,QStringList,QStringList)));
    connect(helpDock, SIGNAL(setStatDocLists(QStringList,QStringList,QStringList)), prjstat, SLOT(setStatDocLists(QStringList,QStringList,QStringList)));
    connect(helpDock, SIGNAL(setStat(QString,QStringList)), prjstat, SLOT(setList(QString,QStringList)));
    //connect(helpDock, SIGNAL(setStat(QString,QTreeWidget)), prjstat, SLOT(setList(QString,QTreeWidget)));
...
void MainWindow::setup()
{
	...
	statusBar()->showMessage(tr("Initializing Research Assistant..."));
	helpDock->initialize();
...
//    connect(ui.actionGoNext, SIGNAL(triggered()), tabs, SLOT(forward()));
	...
	connect(ui.actionTest, SIGNAL(triggered()), browsers()->currentBrowser(), SLOT(insertClipboard()));
	...
	connect(ui.actionProjectStatistics, SIGNAL(triggered()), this, SLOT(showStatistics()));
    ...
    connect(ui.actionAutoText, SIGNAL(triggered()), this, SLOT(showAutoText()));
	...
	connect(ui.actionViewSource, SIGNAL(triggered()), browsers()->currentBrowser(), SLOT(ViewSource()));
...
void MainWindow::about()
{
	...
	QString translator = tr("Localization by: Pavel Malakhov", "Put your name here :)");
	box.setText(QString::fromLatin1("<p align=\"center\"><h3>Research Assistant</h3></p>"
									...
									"<p>%4<p/>")
									.arg(release).arg(info).arg(warranty).arg(translator));
...
void MainWindow::showLink(const QString &link)
{
	...
	if( (!lnk.isEmpty()) && fi.exists() && fi.isFile() ){
		...
		if (ui.actionSaveFile->isEnabled()) //i.e. document was modified
			emit saveOpenedLink();
		QUrl url(link);
		qDebug() << "- tabs->setSource: " << url.toString();
		tabs->setSource(url.toString());
		tabs->currentBrowser()->setFocus();
	}else{
	   qWarning() << "Failed to open link: " << link;
	   QMessageBox::warning(this, "Research Assistant", tr("failed to open file:\n%1").arg(lnk));
	}
}
...
void MainWindow::ProjectNew()
{
	...
	if (!fn.isEmpty()){
		...
		if (fi.suffix().isEmpty())
			fn += ".rap";
		QString sp = fi.absolutePath() + "/about.html";
...
//-------------------------------------------------
void MainWindow::showAutoText()
{
    autotxt->show();
}

//-------------------------------------------------
void MainWindow::addSignature(QString str)
{
    autotxt->addSignature(str);
}
...
void MainWindow::ProjectBackup()
{
	...
	archiveFN = QInputDialog::getText(this, tr("Archive file name"),archiveFN,
										  QLineEdit::Normal, archiveFN, &ok);
	if (ok && !archiveFN.isEmpty()){
		...
		QMessageBox::information(this, tr("Backup"), tr("Archive successfully created in\n%1").arg(archiveFN), tr("OK"));
	}
}
