...
:rootNode(0)
{
  TagString[t_none]    = QString("");
  TagString[t_feed]    = QString("feed");
  TagString[t_language] = QString("language");
  TagString[t_drssFeedList] = QString("drssfeedlist");
  TagString[t_xmlLink] = QString("xmlUrl");
  TagString[t_webLink] = QString("webUrl");
  TagString[t_title]   = QString("title");
  TagString[t_descr]   = QString("description");
  TagString[t_imgLink] = QString("imageUrl");
  TagString[t_nick]    = QString("nick");
  TagString[t_dir]     = QString("folder");
...
FeedList::RecurseDrssDir (QDomElement &el)
{
  ...
  for (QDomElement child = el.firstChildElement();
     ...
     if (t == TagString[t_feed]) {
       ParseFeed (child);
     } else if (t == TagString[t_dir]) {
...
FeedList::Load (QIODevice *dev)
{
  ...
  if (ok) {
    ...
    QString tag = docElem.tagName();
    if (tag == TagString[t_drssFeedList]) {
...
FeedList::ParseFeed (QDomElement & feedEl)
{
   ...
   while (!el.isNull()) {
     ...
     FeedTag ftag = MatchTag(tag);
     switch (ftag) {
     case t_xmlLink:
       pRF->SetXmlLink (el.text());
       break;
     case t_webLink:
       pRF->SetWebLink (el.text());
       break;
     case t_title:
       pRF->SetTitle (el.text());
       break;
     case t_descr:
       pRF->SetDescr (el.text());
       break;
     case t_imgLink:
       pRF->SetImageLink (el.text());
       break;
     case t_nick:
       pRF->SetNick (el.text());
       break;
     case t_language:
       pRF->SetLanguage (el.text());
       break;
     default:
...
void
FeedList::AppendTextElement (QDomElement &del, QString tag, QString text)
{
...
void
FeedList::FillInDom (QDomElement & del, RssFeed & rf)
{
   AppendTextElement (del, TagString[t_language], rf.Language());
   AppendTextElement (del, TagString[t_xmlLink], rf.XmlLink());
   AppendTextElement (del, TagString[t_title], rf.Title());
   AppendTextElement (del, TagString[t_descr], rf.Descr());
   AppendTextElement (del, TagString[t_imgLink], rf.ImageLink());
   AppendTextElement (del, TagString[t_nick], rf.Nick());
   AppendTextElement (del, TagString[t_webLink], rf.WebLink());
}
...
FeedList::Domify ()
{
   ...
   emptyDoc.append("\n");
   emptyDoc.append ("<" + TagString[t_drssFeedList]);
   ...
   for (int row = 0; row < nrows; row++) {
     ...
     QString t = (pData->Dir() ? TagString[t_dir] : TagString[t_feed]);
     el = dom.createElement (t);
     if (pData->Dir()) {
       el.setAttribute ("name",pData->Nick());
       DomifyRecurseDir (el,pRF);
     } else {
       // fill in from pRF
       FillInDom (el,*pData);
     }
     el.setAttribute("hash",pData->Hash());
     root.appendChild(el);
   }
}
...
FeedList::DomifyRecurseDir (QDomElement & dir, QStandardItem * pParent)
{
  ...
  for (int row=0; row< nrows; row++) {
     ...
     QString t = (pChildData->Dir() ? TagString[t_dir] : TagString[t_feed]);
     el = dom.createElement (t);
     if (pChildData->Dir()) {
       el.setAttribute ("name",pChildData->Nick());
       DomifyRecurseDir (el, pChild);
     } else {
       FillInDom (el,*pChildData);
     }
     el.setAttribute("hash",pChildData->Hash());
     dir.appendChild(el);
  }
}

FeedList::FeedTag
FeedList::MatchTag (QString t)
{
  int tg;
