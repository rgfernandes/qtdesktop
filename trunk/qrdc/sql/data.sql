...
CREATE TABLE IF NOT EXISTS c (	id		INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,	name		TEXT NOT NULL,	protoid		INTEGER NOT NULL,	hostid		INTEGER NOT NULL,	port		INTEGER NULL,	varid		INTEGER NULL,	cmdline		TEXT NULL,	comments	TEXT NULL	);
CREATE TRIGGER IF NOT EXISTS p_d	BEFORE	DELETE	ON main.p	FOR EACH ROW	BEGIN		DELETE FROM v WHERE protoid=old.id;		DELETE FROM c WHERE protoid=old.id;	END;
CREATE TRIGGER IF NOT EXISTS v_d	BEFORE	DELETE	ON main.v	FOR EACH ROW	BEGIN		UPDATE c SET varid = NULL WHERE varid=old.id;	END;
CREATE TRIGGER IF NOT EXISTS h_d	BEFORE	DELETE	ON main.h	FOR EACH ROW	BEGIN		DELETE FROM c WHERE hostid=old.id;	END;
