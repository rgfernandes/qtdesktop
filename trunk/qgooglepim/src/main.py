...
import		core.blog, core.code
import		cs_gdata.blog, cs_gdata.contacts, cs_gdata.notebook
...
class	MainWindow(QtGui.QMainWindow, Ui_main.Ui_MainWindow):
	...
	def	__setSlots(self):
		...
		self.actionCode.connect( self.actionCode, QtCore.SIGNAL( "triggered()" ), self.__slOpenCode )

	def	__slOpenBlog(self):
		self.blog = blog.BlogDialog(str(settings.Setting.Login), str(settings.Setting.Password))
		self.blog.exec_()

	def	__slOpenContacts(self):
		self.contacts = cs_gdata.contacts.Contacts()
		self.contacts.login(str(settings.Setting.Login), str(settings.Setting.Password))
		self.contacts.load()

	def	__slOpenNotebook(self):
		self.notebook = cs_gdata.notebook.Notebook()
		self.notebook.login(str(settings.Setting.Login), str(settings.Setting.Password))
		self.notebook.load()

	def	__slOpenCode(self):
		self.code = code.CodeDialog(str(settings.Setting.Login), str(settings.Setting.Password))
		self.code.exec_()
