...
void imagepubPlugin::actionStart()
{
	if (!event_item.m_item_name.isEmpty()) {
		...
		uploadwidget->setStatus(tr("Choose image file"));
		QString filepath = QFileDialog::getOpenFileName(uploadwidget, tr("Choose image"), settings.value("main/lastdir").toString(), tr("Image Files (*.png *.jpg *.gif)"));
		QFile file(filepath);
...
{
	uploadwidget->setStatus(tr("Sending image URL..."));
	if (reply->rawHeader("Location").length()>0) {
		...
		if (rx.indexIn(page)>-1) {
			...
			else msgtemplate = tr("Image sent: %N (%S bytes)\n%U");
			...
			uploadwidget->setStatus(tr("Image URL sent"));
